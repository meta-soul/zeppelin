...
function NoteCreateCtrl($scope, noteListFactory, $routeParams, websocketMsgSrv) {
  ...
  vm.createNote = function() {
    if (!vm.clone) {
      ...
      vm.websocketMsgSrv.createNotebook($scope.note.notename, defaultInterpreterGroup, $scope.workspace);
      $scope.note.defaultInterpreter = $scope.interpreterSettings[0];
    } else {
      let noteId = $routeParams.noteId;
      vm.websocketMsgSrv.cloneNote(noteId, $scope.note.notename, $scope.workspace);
    }
  };
